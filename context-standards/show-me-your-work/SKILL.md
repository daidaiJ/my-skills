...
description: >
  ...
  log with one row per decision (what, why, evidence, result). Local by
  default; commit it when a reviewer needs the trail to trust the result. Use
  for "show me your work", autonomous or multi-phase runs, or work a human
  reviews after stepping away.
metadata:
  short-description: "TSV decision-log trail for long/unattended runs"
provenance:
  origin: cursor-plugins (pstack: show-me-your-work)
  license: MIT
  upstream_url: https://github.com/cursor/plugins/tree/main/pstack/skills/show-me-your-work
...
Write each entry the way you'd tell a teammate what you did. Plain words, concrete actions, no AI speak or abstract jargon (the **stop-slop** skill applies to log text too).
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript when the harness exposes one (a transcript directory or log the system names). Never glob wider than this run's own transcript — that reads unrelated private sessions. If no transcript is available, audit the log against the artifacts it cites instead. Walk the log against what actually happened:
...
Before handing back, spawn a reviewer subagent — on a different model family from the one that did the work when the harness allows it. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript (or the cited artifacts), then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
