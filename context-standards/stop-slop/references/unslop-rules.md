...
Merged from the `unslop` skill (cursor/plugins, MIT). Rule numbers are stable
ids that other skills cite; a removed rule leaves a gap. The Core Rules and
Quick Checks in SKILL.md cover the highest-frequency tells — this catalog is
the full scan list for drafting, editing, or reviewing prose.

Process: scan for the patterns below → rewrite (preserve meaning, match
intended tone) → self-audit: "What makes this obviously AI generated?" Fix
remaining tells.
