...
name: tdd
description: >
  Use only when the user explicitly asks for TDD, a failing test, or a
  regression test, OR when the bug has an obvious cheap local test target.
  Skip when the test path is unclear, expensive, integration-heavy, or not
  requested.
metadata:
  short-description: "Bug fix with a failing-first regression test"
provenance:
  origin: cursor-plugins (pstack: tdd)
  license: MIT
  upstream_url: https://github.com/cursor/plugins/tree/main/pstack/skills/tdd
