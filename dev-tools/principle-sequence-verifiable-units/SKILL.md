...
description: >
  ...
  how you stack commits and PRs. Break work into small units that each end in
  a verifiable state, check each before the next, and order delivery so the
  sequence proves itself to a reviewer.
metadata:
  short-description: "Small units, each ending verifiable"
provenance:
  origin: cursor-plugins (pstack: principle-sequence-verifiable-units)
  license: MIT
  upstream_url: https://github.com/cursor/plugins/tree/main/pstack/skills/principle-sequence-verifiable-units
...
The sequencing complement to the **prove-it-works** principle skill, which keeps each check real; a small script that performs the edits makes the per-unit check nearly free.
