...
name: control-cli
description: >
  Build or adapt a local harness to drive, inspect, and profile an interactive
  CLI or TUI without external services. Use for CLI UX checks, startup
  regressions, memory leaks, hangs, prompt flows, or terminal demos.
metadata:
  short-description: "Local PTY/tmux harness to drive & profile CLIs"
provenance:
  origin: cursor-plugins (cursor-team-kit: control-cli)
  license: MIT
  upstream_url: https://github.com/cursor/plugins/tree/main/cursor-team-kit/skills/control-cli
...
- PTY probe: use a short Python, Node, or Expect script when tmux is unavailable (the default path on Windows).
...
Use a PTY script when you need deterministic waits in a repo that does not have tmux or a demo harness. Keep it temporary unless the user asks to add a reusable test. On Windows use WSL, or swap `pty` for a `subprocess` pipe loop with `WINCH`-free polling — expect slightly weaker TUI fidelity.
