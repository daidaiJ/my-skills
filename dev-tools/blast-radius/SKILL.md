...
metadata:
  short-description: "Pre-ship breakage analysis with proven safety facts"
provenance:
  origin: cursor-plugins (pstack: blast-radius)
  license: MIT
  upstream_url: https://github.com/cursor/plugins/tree/main/pstack/skills/blast-radius
...
This is the complement to reading the diff itself: the diff tells you what changed, blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits with `gh pr diff` / `git log`.
...
6. For a big or wide change, fan out parallel reviewer subagents on the same question and merge the answers (see the graft rules in **dispatch**). Different agents catch different real bugs.
